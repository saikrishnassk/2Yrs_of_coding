...
int main()
{
	...
	while(t--)
	{
		...
		flag[1]=check%2;
		if(minx%2==maxx%2&&maxx%2==0)
			{  counte=(maxx-minx)/2+1;counto=(maxx-minx)/2; }
		else if(minx%2==maxx%2&&maxx%2==1)
             { counto=(maxx-minx)/2+1;counte=(maxx-minx)/2; }
        else
        	 counto=counte=(maxx-minx)/2+1;
